...
def FFDRatio(items, binSize, ratio, runTime, dataName):
    ...
    if len(binSize) == 1:
        ...
        for i in range(runTime):
            res.append(FFDRatio4D(itemsCopy, binSize, ratio))
    elif len(binSize) == 6:
        for i in range(runTime):
            res.append(FFDRatio6D(itemsCopy, binSize, ratio))
    else:
...
def FFDRatio4D(itemsList, binSize, ratio):
    # az arányszám (ratio) függvényében a végéről rakunk el egy tárgyat
    bins = []  # Felhasznált ládák listája
    binsIndex = 0  # A ládák indexelésére
...
def FFDRatio6D(itemsList, binSize, ratio):
    # az arányszám (ratio) függvényében a végéről rakunk el egy tárgyat
    bins = []  # Felhasznált ládák listája
    binsIndex = 0  # A ládák indexelésére
