...
class Bin2D:
    def __init__(self, index, D1, D2):
        ...
        self.sumFreeCapacity = D1 + D2

        self.itemDB = 0  # Ennyi tárgy van a ládában jelenleg
        self.loadSum = 0  # Össz töltöttség

    def __str__(self):
        return f"Láda száma: [{self.binIndex}] Max kapacitások: [{self.D1},{self.D2}] " \
               f"Töltöttségek: [{self.d1Load},{self.d2Load}]  " \
               f"Üres helyek: [{self.d1FreeCapacity},{self.d2FreeCapacity}] Sum: [{self.sumFreeCapacity}] " \
    ...
    def addItem(self, item):
        self.items.append(str(item.number) + " " + str(item.getD1()) + " " + str(item.getD2()))  # mivan ha itt itemet adok át?
        self.d1Load += item.getD1()
        self.d2Load += item.getD2()
        self.loadSum = self.d1Load + self.d2Load

        self.d1FreeCapacity -= item.getD1()
        self.d2FreeCapacity -= item.getD2()
        self.sumFreeCapacity = self.d1FreeCapacity + self.d2FreeCapacity
